...
impl GpuSoABackend {
    ...
    pub(super) fn read_active_count(&self) -> AriaResult<u32> {
        ...
        rx.recv()
            .map_err(|e| AriaError::gpu(format!("Counter map failed: {}", e)))?
            .map_err(|e| AriaError::gpu(format!("Counter map error: {:?}", e)))?;
    ...
    pub fn read_lifecycle_counters(&self) -> AriaResult<(u32, u32, u32, u32)> {
        ...
        rx.recv()
            .map_err(|e| AriaError::gpu(format!("Lifecycle counters map failed: {}", e)))?
            .map_err(|e| AriaError::gpu(format!("Lifecycle counters map error: {:?}", e)))?;
